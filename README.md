...
**2026-09-08 / 0.5 · [카메라 따라 하기](docs/CAMERA_MIRROR_0.5.md)**

`Start_PocketPal.command` 실행 → [나 따라 해 봐](http://127.0.0.1:8765/mirror/index.html).
사람·티라노·하마·돼지와 핑크·개구리 6종으로 손 들기 반응, 좌우 위치·기울기 따라가기를 시험합니다.
마우스와 글 입력에도 반응하며 카메라 없는 시연 버튼을 제공합니다. MediaPipe 런타임과 모델을 포함해 영상은 브라우저 안에서 처리합니다.
팔·다리 관절의 정밀 복제, 표정 인식, 일상 활동의 의미 이해는 아직 구현하지 않았습니다.
